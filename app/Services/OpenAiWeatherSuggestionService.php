    ...
    public function getSuggestions($userId, $items, $weatherData)
    {
        try {
            ...
            $itemData = collect($items)->map(function ($item) {
                return "Item: {$item->item_name}, Type: {$item->clouth_type}, Material: {$item->material}, Pattern: {$item->pattern}, Color: {$item->color}, Season: {$item->season}, Image: {$item->image}, ID: {$item->id}";
            })->implode("\n");
